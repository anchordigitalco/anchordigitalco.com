...
export default function IndustryCard({ icon, title, description, index = 0 }: IndustryCardProps) {
  ...
  return (
    <motion.div
      ...
      className="industry-card group relative bg-charcoal-800 border border-charcoal-600 p-6 rounded-lg overflow-hidden cursor-default transition-[border-color,box-shadow,transform] duration-200 hover:border-gold/40 shadow-surface hover:shadow-gold"
    >
      {/* Blue top border on hover */}
      <div className="absolute top-0 left-0 right-0 h-0.5 bg-gold scale-x-0 group-hover:scale-x-100 transition-transform duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] origin-left rounded-t-lg" />
      ...
      {/* Description */}
      <p className="font-sans text-sm text-charcoal-200 leading-relaxed font-400">
        {description}
      </p>
    </motion.div>
  )
