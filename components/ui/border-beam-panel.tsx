...
/* Border Beam Panel — from Motiq (https://motiq.dev/components/border-beam-panel).
   ...
   Adapted like every other components/ui/* primitive in this project: the
   root panel's own default surface (border/background/text) is remapped from
   Motiq's hardcoded dark-UI hex values to this project's real light-theme
   tokens (border-hairline/bg-ground/text-ink), since that's the sensible
   default here — callers on a dark section (the services page's "Our
   process" card) override it with bg-dark/text-dark-ink via className, same
   as everywhere else `dark` sections opt out of the light defaults. Also
   drops the file's own hand-rolled `cn()` in favor of this project's shared
   one (@/lib/utils, already clsx+tailwind-merge — same dependencies this
   component would otherwise have asked to install).

   Left untouched: the physics (Spring), the comet/ring conic-gradient math,
   the reduced-motion and visibility-pause hooks, and the MOTIQ_TOKENS block
   that drives the beam's own accent colors — none of that references
   anything that needed remapping, and it conveniently already keys off
   `[data-theme="dark"]`, which is exactly how this project marks its own
   dark sections, so the beam automatically picks up its darker palette
   inside e.g. the Hero without any extra wiring. */
...
@media (forced-colors: active) {
  ...
  return (
    <div
      ...
      onBlur={settle}
      className={cn(
        "relative w-full border border-hairline bg-ground p-7 text-ink",
        cls,
        className,
      )}
      style={{
