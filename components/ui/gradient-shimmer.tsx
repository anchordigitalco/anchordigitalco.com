...
/**
 * Ported from the shadcn community "gradient-shimmer" component, unmodified —
 * it's already framework-agnostic (inline styles + the Web Animations API,
 * no shadcn CSS-variable classes, no named Tailwind radius), so unlike the
 * other adapted components in this project it needed no token remapping.
 * Used on the homepage hero's "Anchor" / "Digital" wordmark.
 */
...
export function GradientShimmer({
  ...
  useEffect(() => {
    ...
    // Static, no sweep: leave the seeded full-text gradient (backgroundSize
    // 100% 100%, no explicit backgroundPosition) alone rather than calling
    // measure() — measure() narrows backgroundSize to the moving band's own
    // width, which without an animation to also move backgroundPosition off
    // its browser-default 0%/left, would freeze on just a slice of the
    // gradient instead of a clean static spread across the whole word.
    if (respectReducedMotion && prefersReducedMotion()) return
    if (typeof el.animate !== 'function') return

    // Refine the seeded vars with a real measurement now that a sweep will
    // actually run.
    measure()
